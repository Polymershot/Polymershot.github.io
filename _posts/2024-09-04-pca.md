...
\\
\large{\lVert x_i  \rVert} \longrightarrow \large{\sqrt{x^2_1 + x^2_2 + ...}:}  \hspace{0.5cm} \text{Basically, square the coordinates of the data points and add them all up; then take the square root at the end}\\
\\
\large{\vec{v_1}:} \hspace{0.5cm} \text{A unit vector onto which $\large{x_i}$ will be projected on}\\
\large{x'_i = \langle x_i, v_1 \rangle v_1 :} \hspace{0.5cm} \text{The projection of the data point (vector) onto the $\vec{v_1}$ unit vector which is a coordinate and not a scalar}\\
\large{x_i - x'_i = e_i:} \hspace{0.5cm} \text{Think of this as like the residuals}\\
\large{a_i{_1} = \langle x_i, v_1 \rangle:} \hspace{0.5cm} \text{The "length" of the projection of the data point (vector) onto the $\vec{v_1}$ unit vector which is a scalar}
\end{align}
$$
</p>
...
$$
\begin{align}
\large{ {\lVert {\vec{x_i} - (\vec{x_i} \cdot \vec{w})} \vec{w} \rVert}^2 } &=& \large{ (\vec{x_i} - (\vec{x_i} \cdot \vec{w}) \vec{w}  ) \cdot (\vec{x_i} - (\vec{x_i} \cdot \vec{w}) \vec{w})} \hspace{2cm}  \text{Squared distance from the data point to its projection onto $\vec{w}$} \\
...
&=& \large{ {\lVert \vec{x_i} \rVert}^2 - 2(\vec{x_i} \cdot \vec{w})^2 + (\vec{x_i} \cdot \vec{w})^2 \vec{w} \cdot \vec{w}} \hspace{2cm} \text{${\lVert \vec{x_i} \rVert}^2$ can also be written as $\vec{x_i} \cdot \vec{x_i}$} \\
...
&=& \large{\frac{1}{n} (XW)^T (XW) } \hspace{2cm} \text{X and W are now matrices; X is a nxp matrx while W is a px1 matrix (n=rows of data, p = columns)} \\
&=& \large{\frac{1}{n} W^T X^T XW} \\
&=& \large{W^T \  \frac{X^T X}{n} \ W} \\
&& \large{\frac{X^T X}{n} \ W} \hspace{2cm} \text{This is your covariance matrix since in ${(x - \mu)^2 \over n}$, each column has a mean of 0} \\
&=& \large{W^T V  W}  \hspace{2cm} \text{V is the covariance matrix of the data} \\
\end{align}
$$
</p>
...
\large{W^T W} &=& \large{1} \hspace{2cm} \text{Set the partial derviateives to 0 to solve for the unknown variables} \\[30pt]
...
/07%3A_Spectral_Theory/7.04%3A_Orthogonality#:~:text=where%20D%20is%20a%20diagonal,is%20in%20fact%20orthogonally%20diagonalizable."

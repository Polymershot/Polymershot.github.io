...
&=& \large{ {\lVert \vec{x_i} \rVert}^2 - 2(\vec{x_i} \cdot \vec{w})^2 + (\vec{x_i} \cdot \vec{w})^2 \vec{w} \cdot \vec{w}} \hspace{1.5cm} \text{${\lVert \vec{x_i} \rVert}^2$ can also be written as $\vec{x_i} \cdot \vec{x_i}$} \\

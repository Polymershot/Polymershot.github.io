...
$$
\begin{align}
\large{ {\lVert {\vec{x_i} - (\vec{x_i} \cdot \vec{w})} \vec{w} \rVert}^2 } &=& \large{ (\vec{x_i} - (\vec{x_i} \cdot \vec{w}) \vec{w}  ) \cdot (\vec{x_i} - (\vec{x_i} \cdot \vec{w}) \vec{w})} \hspace{5cm}  \text{Squared distance from the data point to its projection onto $\vec{w}$} \\
